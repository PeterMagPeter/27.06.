...
export default function TeamGamePrototype() {
  ...
  const [delPartnerItems, setDelPartnerItems] = useState<Array<Layout>>([]);
    ...
    socket.on(
      ...
      (
        ...
        if (partnerTeam === team && username !== playerName) {
          // console.log("socket Changed ships", rotated);
          // if (rotated === true) {
          //   console.log("rotate update ship: ", ships[0])
          //   updatePartnerRotatedShip(ships[0]);
          // } else {
  ...
  const rotateAdd = () => {
    ...
    let newShipArray = [newShip];

    setItems(newShips);
    if (gameMode === "Team") sendShipsToPartner(newShipArray, true);
    setDelItems([]);
  };
  ...
  const removePartnerShip = (id: string) => {
    ...
    // Finde das zweite Item im Array
    const secondItemIndex = partnerShips.findIndex(
      (item, index) => id === item.i
    );
    // console.log(
    //   "delItem ",
    //   secondItemIndex,
    //   partnerShips[secondItemIndex],
    //   partnerShips
    // );
    setDelPartnerItems([partnerShips[secondItemIndex]]);
    if (secondItemIndex !== -1) {
      // Entferne das Item aus dem items-Array
      const updatedItems = [...partnerShips];
      updatedItems.splice(secondItemIndex, 1);
      // Aktualisiere den Zustand mit dem aktualisierten Array
  ...
  const addPartnerShips = () => {
    // console.log("addPArtner start ", delPartnerItems[0], partnerShips, layout);
    let newX = delPartnerItems[0].x;
    let newY = delPartnerItems[0].y;
    let newH = delPartnerItems[0].h;
    let newW = delPartnerItems[0].w;
    let newShip: any = {
      i: delPartnerItems[0].i,
      x: newX,
      y: newY,
      w: newW,
      h: newH,
      static: false, 
      id: delPartnerItems[0].id,
    };
    newShip = findNonOverlappingPosition(newShip);
    // Fügen Sie das Schiff zum items-Array hinzu
    let newShips = [...partnerShips, newShip]; // alters schiff layout  wird genommen 
    // console.log("addPartner", newShips)
    setPartnerShips(newShips);
    setDelPartnerItems([]);
  }; 
  useEffect(() => {
  ...
  useEffect(() => {
    if (delPartnerItems.length === 1) {
      addPartnerShips();
    }
  }, [delPartnerItems, layout, partnerShips]);
  ...
  function sendShipsToPartner(arr: Layout[], rotated: boolean = false): void {
  ...
  function updatePartnerRotatedShip(ship: ShipTemplate) {
    const translatedPartnerShip = translateToLayout(ship);
    // console.log("send to remove ", translatedPartnerShip)
    removePartnerShip(translatedPartnerShip.i);
  }
  function updatePartnerShips(ships: ShipTemplate[]) {
    ...
    setLayout((prevLayout) => {
      return prevLayout.map((item) => {
        const updatedShip = translatedPartnerShips.find(
          (ship) => ship.i === item.i
        );
